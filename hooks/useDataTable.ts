...
interface UseDataTableOptions<T> {
  api:
    ...
    | {
        ...
        downloadItems?: (params: any) => Promise<void>;
        uploadSerials?: (file: File) => Promise<any>;
        uploadSettlement?: (file: File) => Promise<any>;
        uploadTerminalIds?: (file: File) => Promise<any>;
      };
...
export function useDataTable<T>({
  ...
  const handleExport = useCallback(async (params?: any) => {
    ...
    await apiRef.current.downloadItems({
      search: params?.search,
      ...params?.filters,
    });
  }, []);
  ...
  const handleUpload = useCallback(
    async (
      file: File,
      uploadMethod: 'uploadSerials' | 'uploadSettlement' | 'uploadTerminalIds',
  ...
  return {
    ...
    handleUpload:
      (apiRef.current as any).uploadSerials ||
      (apiRef.current as any).uploadSettlement ||
      (apiRef.current as any).uploadTerminalIds
        ? handleUpload
        : undefined,
    // handleUpload:
    //   (apiRef.current as any).uploadSerials ||
